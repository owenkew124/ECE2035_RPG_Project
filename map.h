...
/**
 * Add a MON item at (x,y). If there is already a MapItem at (x,y), erase it
 * before adding the monster.
 */
void add_mon(int x, int y);
/**
 * Add a PORTAL item at (x,y). If there is already a MapItem at (x,y), erase it
 * before adding the portal.
 */
...
/**
* Gives the numerical representation of the active map
*
*/
int get_active_map_num();
/**
* Moves the npc in a random direction depending on the rand()
* function
*/
int update_npc(int x, int y, int move);
/**
 * Add RIVER items in a line of length len beginning at (x,y).
 * If dir == HORIZONTAL, the line is in the direction of increasing x.
 * If dir == VERTICAL, the line is in the direction of increasing y.
 *
 * If there are already items in the map that collide with this line, they are
...
/**
 * Add a BRIDGE item at (x,y). If there is already a MapItem at (x,y), erase it
 * before adding the bridge.
 */
void add_bridge(int x, int y, int dir, int len);
/**
 * Add a BWALL item at (x,y). If there is already a MapItem at (x,y), erase it
 * before adding the bridge wall.
 */
void add_bridge_wall(int x, int y, int dir, int len);
/**
 * Add a TRAP item at (x,y). If there is already a MapItem at (x,y), erase it
 * before adding the trap.
 */
